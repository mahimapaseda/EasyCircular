  ...
  if (orientation === "vertical") {
    return (
      <nav aria-label="Workflow progress" className="relative py-1">
        <div
          className="absolute left-[15px] w-0.5 -translate-x-1/2 rounded-full bg-white/15"
                ...
                <div className="min-w-0">
                  <p
                    className={`text-sm font-bold leading-tight ${
                      active ? "text-white" : done ? "text-cyan-200" : "text-slate-500"
                    }`}
                  >
                    {step.label}
                  </p>
  ...
  return (
    <nav aria-label="Workflow progress" className="w-full">
      <div className="relative">
        <div
          className="absolute top-3.5 h-0.5 -translate-y-1/2 rounded-full bg-white/15"
              ...
              <li key={step.key} className="flex flex-1 flex-col items-center gap-1.5">
                ...
                <span
                  className={`text-center text-[10px] font-bold uppercase tracking-[0.14em] ${
                    active ? "text-white" : done ? "text-cyan-300" : "text-slate-500"
                  }`}
                >
                  {step.label}
                </span>
...
function Marker({ done, active, id }: { done: boolean; active: boolean; id: number }) {
  if (active) {
    return (
      <span className="relative z-10 flex h-7 w-7 shrink-0 items-center justify-center">
        ...
        <span className="relative flex h-7 w-7 items-center justify-center rounded-full border-2 border-cyan-300 bg-slate-950 text-[11px] font-bold text-white shadow-[0_0_0_4px_rgba(34,211,238,0.12)]">
          {id}
        </span>
      </span>
    );
  ...
  return (
    <span className="relative z-10 flex h-7 w-7 shrink-0 items-center justify-center">
      <span className="flex h-5 w-5 items-center justify-center rounded-full border border-white/20 bg-slate-950 text-[10px] font-bold text-slate-500">
        {id}
      </span>
    </span>
  );
