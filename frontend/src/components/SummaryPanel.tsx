    ...
    <section className="panel-gradient overflow-hidden">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-fuchsia-500 to-violet-600 shadow-md shadow-fuchsia-500/30">
        ...
        <div className="flex flex-wrap gap-2">
          {summary && (
            <>
              <button type="button" onClick={() => handleExport("txt")} className="btn-secondary text-xs px-3 py-1.5 gap-1.5">
                <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                </svg>
                TXT
              </button>
              <button type="button" onClick={() => handleExport("md")} className="btn-secondary text-xs px-3 py-1.5 gap-1.5">
                <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      {summary && (
        <div className="mt-5 space-y-4">
          <div className="flex items-center gap-2">
            <h3 className="text-lg font-black text-ink-900 dark:text-white">{summary.title}</h3>
            {summary.mode === "llm" && (
              <span className="rounded-full bg-gradient-to-r from-brand-100 to-fuchsia-100 px-2.5 py-0.5 text-xs font-bold text-brand-700 ring-1 ring-brand-200 dark:from-brand-950/50 dark:to-fuchsia-950/30 dark:text-brand-300 dark:ring-brand-700">
                AI-generated
              </span>
            )}
            {summary.mode === "fallback" && (
              <span className="rounded-full bg-amber-100 px-2.5 py-0.5 text-xs font-bold text-amber-700 ring-1 ring-amber-200 dark:bg-amber-950/40 dark:text-amber-300 dark:ring-amber-700">
                Extractive
              </span>
            )}
          </div>
          ...
          <div className="space-y-3">
            {summary.sections.map((section, index) => (
              <div
                key={section.heading}
                className={`rounded-xl border-l-4 p-3.5 ${SECTION_COLORS[index % SECTION_COLORS.length]} ${DARK_SECTION_COLORS[index % DARK_SECTION_COLORS.length]} to-transparent`}
              >
                <h4 className="text-xs font-black uppercase tracking-wider text-ink-500 dark:text-ink-400">
                  {section.heading}
                </h4>
          ...
          {summary.actionItems.length > 0 && (
            <div className="rounded-xl border-2 border-emerald-200 bg-gradient-to-br from-emerald-50 to-teal-50/60 p-4 dark:border-emerald-800/50 dark:from-emerald-950/30 dark:to-teal-950/20">
              <h4 className="flex items-center gap-2 text-xs font-black uppercase tracking-wider text-emerald-700 dark:text-emerald-400">
              ...
              <ul className="mt-3 space-y-2">
                {summary.actionItems.map((item, i) => (
                  <li key={item} className="flex items-start gap-2.5 text-sm text-ink-700 dark:text-ink-200">
                    <span className="mt-1 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-emerald-500 text-[10px] font-black text-white">
                      {i + 1}
                    </span>
                    {item}
                  </li>
